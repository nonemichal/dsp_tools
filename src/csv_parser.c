...
Vector_csv vector_from_file(const char *path, const char delimiter) {
    ...
    // create empty instance of Vector_csv
    ...
    while ((ch = (char)fgetc(fptr)) != EOF) {
        ...
        if (val_counter > VAL_BUFFER_SIZE) {
            printf("Buffer size can not be geater than max value buffer size: "
                   "%zu.\nReturning an empty vector",
                   VAL_BUFFER_SIZE);
            fclose(fptr);
            free(val_buffer);
        ...
        if (ch != delimiter) {
            ...
            chars_to_float[char_counter] = ch;
            ...
            val_buffer[val_counter] = strtof(chars_to_float, 0);
